...
在快照中，有一部分节点可以直接通过 `id` / `vid` / `name` 定位，不需要遍历就能获取。[^2] 这部分节点在树形图中是加粗显示的。
...
```ts
@[text="关闭"][clickable=true] + [text="活动"]
```
...
```ts
@[text="关闭"][clickable=true] + [text="活动"] <<n [vid="browser_fragment_layout"]
```
